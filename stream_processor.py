...
# 1. Initialize Spark
spark = SparkSession.builder.appName("CompressionAnomalyDetector").getOrCreate()
...
# Kafka sends data as binary bytes in the 'value' column. We cast to string and parse.
...
analyzed_stream = parsed_stream.withColumn(
    "alert_message",
    when(
        col("payload_bytes") > 10,
        "CRITICAL: Compression Ratio Drop Detected! (Uncompressed Data Found)",
    ).otherwise(None),
)
...
kafka_output = alerts.select(
    col("sensor_id").alias("key"), col("alert_message").alias("value")
)
