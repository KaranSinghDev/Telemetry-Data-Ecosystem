...
class DashboardWindow(QMainWindow):
    def __init__(self):
        ...
        self.crosshair_v = pg.InfiniteLine(
            angle=90, movable=False, pen=pg.mkPen("k", style=Qt.PenStyle.DashLine)
        )
        self.crosshair_h = pg.InfiniteLine(
            angle=0, movable=False, pen=pg.mkPen("k", style=Qt.PenStyle.DashLine)
        )
    ...
    # All other functions (create_stat_box,
    # on_mouse_move, run_query_and_update_plot, etc.)
    ...
    def create_stat_box(self, name):
        ...
        value.setStyleSheet(
            "background-color: #f0f0f0; border: 1px solid #ccc; font-weight: bold;"
        )
    ...
    def on_mouse_move(self, event):
        if self.plot_item.vb.sceneBoundingRect().contains(event):
            ...
            time_str = QDateTime.fromSecsSinceEpoch(timestamp).toString("HH:mm:ss")
            value_val = mouse_point.y()
            label_text = f"Time: {time_str}, Value: {value_val:.2f}"
    ...
    def run_query_and_update_plot(self):
        try:
            ...
            if start_ts_ms >= end_ts_ms and self.refresh_timer.isActive() is False:
            ...
            self.data_line = self.plot_item.plot(
                timestamps_s, values, pen=pg.mkPen("#0072B2", width=2)
            )
                ...
                axis.setTicks(
                    [
                        [
                            (
                                t,
                                QDateTime.fromSecsSinceEpoch(int(t)).toString(
                                    "HH:mm:ss"
                                ),
                            )
            ...
            self.show_message_box(
                "Connection Error",
                f"Could not connect to the database service at {API_BASE_URL}.\n"
