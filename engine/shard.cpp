  ...
  while (stream.peek() != EOF) {
    ...
    if (prev_timestamp == 0) {
      ...
      int64_t delta_of_delta =
          (zigzag_encoded >> 1) ^ (-(static_cast<int64_t>(zigzag_encoded & 1)));
  ...
  if (!points.empty()) {
    ...
    if (points.size() > 1) {
      ...
      this->prev_timestamp_delta =
          last_point.timestamp - second_last_point.timestamp;
    } else {
      this->prev_timestamp_delta = 0;
    }
  }
...
void ShardWriter::append(const DataPoint &point) {
  ...
  if (this->prev_timestamp == 0) {
    ...
    int64_t delta_of_delta =
        static_cast<int64_t>(current_delta - this->prev_timestamp_delta);
