...
// --- Test Cases ---
// We use TEST_F() to indicate that this test uses the 'ShardedQueryTest'
// Fixture. Each TEST_F is a separate, independent test case.
...
TEST_F(ShardedQueryTest, QueryWithInsufficientBuffer) {
  // This tests that our function correctly handles a full buffer and doesn't
  // write out of bounds.
  int64_t count = query_range(0, 9000000, results_buffer,
                              3); // Buffer only has space for 3 points
  ASSERT_EQ(count, 3);
}
