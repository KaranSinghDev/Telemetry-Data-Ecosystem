...
extern "C" {
// The implementation of the ingest function, which delegates to the ShardWriter
// class.
...
// The implementation of the query function, which delegates to the ShardReader
// class.
int64_t query_range(uint64_t start_ts, uint64_t end_ts, DataPoint *out_buffer,
  ...
  uint64_t first_shard_start =
      (start_ts / SHARD_DURATION_MS) * SHARD_DURATION_MS;
  uint64_t last_shard_start = (end_ts / SHARD_DURATION_MS) * SHARD_DURATION_MS;

  for (uint64_t shard_start = first_shard_start;
       shard_start <= last_shard_start; shard_start += SHARD_DURATION_MS) {
    uint64_t shard_end = shard_start + SHARD_DURATION_MS - 1;
    std::string file_path = DATA_DIRECTORY + "/" + std::to_string(shard_start) +
                            "-" + std::to_string(shard_end) + ".bin";
    ...
    for (const auto &point : points) {
      if (points_found < buffer_capacity && point.timestamp >= start_ts &&
          point.timestamp <= end_ts) {
...
std::string get_shard_path(uint64_t timestamp) {
  ...
  return DATA_DIRECTORY + "/" + std::to_string(shard_start_ts) + "-" +
         std::to_string(shard_end_ts) + ".bin";
}
