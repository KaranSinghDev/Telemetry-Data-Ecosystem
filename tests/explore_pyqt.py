    ...
    def __init__(self):
        ...
        print(
            """  - Click the 'Generate New Data' button
             to see the 'clicked' signal in action."""
        )
        print(
            " - Notice the plot updating every second from  QTimer's 'timeout' signal"
        )

    def update_plot_data(self):
        """This function is called ONLY when the update_button's
...
if __name__ == "__main__":
    ...
    print(
        "Application event loop started. The script will now wait for user interaction."
    )
    sys.exit(app.exec())
