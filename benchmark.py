...
def run_storage_benchmark():
    ...
    compressed_size = sum(
        f.stat().st_size for f in os.scandir(DATA_DIRECTORY) if f.is_file()
    )
    bytes_per_point = compressed_size / NUM_POINTS_ENGINE
    compression_ratio = uncompressed_size / compressed_size
    print(
        f"""RESULT: On-disk size for {NUM_POINTS_ENGINE:,}
        points is {compressed_size / (1024*1024):.2f} MB."""
    )
...
def run_query_benchmark(lib, all_points):
    ...
    for _ in range(NUM_QUERIES_ENGINE):
        start_index = random.randint(
            int(NUM_POINTS_ENGINE * 0.9), NUM_POINTS_ENGINE - 3601
        )
...
def run_full_engine_suite():
    ...
    try:
        ...
        lib.query_range.argtypes = [
            ctypes.c_uint64,
            ctypes.c_uint64,
            ctypes.POINTER(CDataPoint),
            ctypes.c_int64,
        ]
        ...
        print(
            ...
            compiled ('cmake --build build' in 'engine/' directory)."""
        )
        return
...
def run_kafka_benchmark_high_throughput():
    """Benchmarks the Kafka pipeline's maximum ingestion
    throughput using multiple processes."""
    print("\n" + "=" * 60)
    print(
        f"""  Running Kafka Ingestion Stress
        Test (MULTIPROCESS) ({KAFKA_NUM_MESSAGES:,} messages)"""
    )
    print("=" * 60)
